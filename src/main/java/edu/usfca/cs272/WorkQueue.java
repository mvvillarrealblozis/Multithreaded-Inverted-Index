...
public class WorkQueue {
	 ...
	 * @throws IllegalStateException if this method is called when there are no pending tasks
	 */
	public void execute(Runnable task) throws IllegalStateException {
		synchronized (tasks) {
			if (shutdown) {
				throw new IllegalStateException("Task shut down, illegal state");
			}

			tasks.addLast(task);

			tasks.notify(); // TODO notifyAll();
		}
	...
	/**
	 * Decrements the count of pending tasks and notifies waiting threads if no tasks remain
	 */
